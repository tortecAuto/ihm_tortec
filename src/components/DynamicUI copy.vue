...
paper.on('blank:pointermove', (event, x, y) => {
  if (this.isSelecting && startSelectionPoint) {
    ...
    this.selectionBox = {
      x: width > 0 ? startSelectionPoint.x : x,
      y: height > 0 ? startSelectionPoint.y : y,
...
paper.on('element:pointerup', () => {
  startPos = {};
    ...
    // Salvar a posição inicial de todos os elementos selecionados
    ...
    // Não permitir arrasto ou deselecionar todos
    this.selectedElements = []; // Deseleciona todos se clicar fora
    startPos = {};
  }
